...
static XYWIFIManager *shareManager = nil;

@interface XYWIFIManager ()<AsyncSocketDelegate>
// 连接的socket对象
@property (nonatomic,strong) AsyncSocket *sendSocket;
@property (nonatomic,strong) NSTimer *connectTimer;
@end

@implementation XYWIFIManager
+ (instancetype)shareWifiManager {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shareManager = [[XYWIFIManager alloc] init];
    });
    return shareManager;
}
...
- (void)onSocket:(AsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port
{
    ...
    if ([self.delegate respondsToSelector:@selector(XYWIFIManager:didConnectedToHost:port:)]) {
        [self.delegate XYWIFIManager:self didConnectedToHost:host port:port];
    }
//    self.callBack(YES);
    // 每隔30s像服务器发送心跳包
...
- (void)onSocket:(AsyncSocket *)sock didWriteDataWithTag:(long)tag
{
    ...
    if ([self.delegate respondsToSelector:@selector(XYWIFIManager:didWriteDataWithTag:)]) {
        [self.delegate XYWIFIManager:self didWriteDataWithTag:tag];
    }
    [_sendSocket readDataWithTimeout: -1 tag: 0];
}
...
- (void)onSocket:(AsyncSocket *)sock willDisconnectWithError:(NSError *)err
{
    ...
    if ([self.delegate respondsToSelector:@selector(XYWIFIManager:willDisconnectWithError:)]) {
        [self.delegate XYWIFIManager:self willDisconnectWithError:err];
    }
...
- (void)onSocket:(AsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
    ...
    if ([self.delegate respondsToSelector:@selector(XYWIFIManager:didReadData:tag:)]) {
        [self.delegate XYWIFIManager:self didReadData:data tag:tag];
    }
...
- (void)onSocketDidDisconnect:(AsyncSocket *)sock
{
    ...
    _connectOK = NO;
    if ([self.delegate respondsToSelector:@selector(XYWIFIManagerDidDisconnected:)]) {
        [self.delegate XYWIFIManagerDidDisconnected:self];
    }
    if (sock.userData == SocketOfflineByServer) {
        _isAutoDisconnect = YES;
        // 重连
        
        [self XYConnectWithHost:_hostStr port:_port completion:^(BOOL isConnect) {
